...
class UnitPlanViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        """
        print(
            f"Unit plan create - received data keys: {list(request.data.keys())}")
        print(
            f"Unit plan create - received files keys: {list(request.FILES.keys())}")
        if 'document' in request.FILES:
            doc = request.FILES['document']
            print(
                f"Unit plan create - document info: name={doc.name}, size={doc.size}, content_type={doc.content_type}")
        else:
            print("Unit plan create - WARNING: No document file received!")

        # Extra debug info about the request
        print(
            f"Unit plan create - user: {request.user.username}, teacher: {getattr(request.user, 'teacher', None)}")
        ...
        print(
            f"Unit plan create - class_id received: {class_id}, type: {type(class_id)}")

        # Check for invalid string values
        if class_id in ['undefined', 'null', ''] or class_id is None:
            print(
                f"Unit plan create - ERROR: Invalid class_id value: '{class_id}'")
            return Response(
        ...
        if isinstance(class_id, str):
            try:
                if class_id.isdigit():
                    ...
                    print(
                        f"Unit plan create - class_id converted to integer: {class_id}")
                else:
                    print(
                        f"Unit plan create - ERROR: class_id is not a valid integer: '{class_id}'")
                    return Response(
        ...
        try:
            class_obj = Classes.objects.get(
                id=class_id, teacher=request.user.teacher)
            print(
                f"Unit plan create - Found class: {class_obj.class_name} (ID: {class_obj.id})")
        except Classes.DoesNotExist:
            print(
                f"Unit plan create - ERROR: Class not found or permission denied for class_id {class_id}")
            return Response(
            ...
            print(
                f"Unit plan create - ERROR: Unexpected error accessing class: {str(e)}")
            return Response(
        ...
        existing_plan = UnitPlan.objects.filter(
            class_instance=class_obj).first()
        if existing_plan:
            ...
            print(
                f"Unit plan create - Existing plan found, from_creation_flow: {from_creation_flow}")
        ...
        try:
            ...
            print(
                f"Unit plan create - ERROR: Unexpected error in create: {str(e)}")
            return Response(
    ...
    def update_existing_plan(self, request, instance, *args, **kwargs):
        ...
        """
        print(
            f"Unit plan update_existing_plan - Instance ID: {instance.id}, Class ID: {instance.class_instance.id if hasattr(instance, 'class_instance') else 'unknown'}")
        print(f"Unit plan update_existing_plan - Files: {request.FILES}")
        print(
            f"Unit plan update_existing_plan - Data keys: {list(request.data.keys())}")
        ...
        try:
            ...
            if 'from_creation_flow' in request.data:
                print(
                    f"Unit plan update_existing_plan - Removing from_creation_flow: {request.data.get('from_creation_flow')}")
            ...
            if 'document' in request.FILES:
                ...
                if instance.document:
                    try:
                        ...
                        print(
                            f"Unit plan update_existing_plan - Old document path: {old_document}")
                    except Exception as e:
                        print(
                            f"Unit plan update_existing_plan - Error accessing old document: {e}")
                        # Continue anyway
                ...
                try:
                    ...
                    print(
                        f"Unit plan update_existing_plan - New document name: {instance.document.name}")
                except Exception as e:
                    print(
                        f"Unit plan update_existing_plan - Error setting new document: {e}")
                    return Response(
                ...
                if 'title' in request.data:
                    ...
                    print(
                        f"Unit plan update_existing_plan - Updated title: {instance.title}")
                ...
                try:
                    instance.save()
                    print(
                        "Unit plan update_existing_plan - Saved instance with new document")
                except Exception as e:
                    print(
                        f"Unit plan update_existing_plan - Error saving instance: {e}")
                    return Response(
                ...
                if old_document:
                    ...
                    if os.path.exists(old_document):
                        try:
                            ...
                            print(
                                f"Unit plan update_existing_plan - Deleted old document file")
                        except Exception as e:
                            print(
                                f"Unit plan update_existing_plan - Error deleting old document file: {e}")
                            # Continue anyway
                ...
                print(
                    "Unit plan update_existing_plan - No new document, standard partial update")
    ...
    def download(self, request, pk=None):
        ...
        response[
            'Content-Disposition'] = f'attachment; filename="{unit_plan.document.name.split("/")[-1]}"'
        return response
